...
export async function loader({ params, request }: { params: { slug: string }, request: Request }) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Response(`API request failed: ${response.status} ${response.statusText}`, { 
    ...
    if (!product.Produkttitel) {
      throw new Response("Invalid product data: Missing required fields", { status: 500 });
    }
...
export default function ProductRoute() {
  ...
  const handleAddToCart = () => {
    ...
    if (!selectedVariant) {
      ...
      setConfigurationError('Um Ihnen das bestmögliche Produkt anbieten zu können, wählen Sie bitte zuerst Ihre gewünschte Konfiguration aus.');
      return;
    }
    ...
    const missingAttributes = Array.from(requiredAttributes)
      ...
      .map(attr => {
        ...
        switch(attr) {
          ...
          case 'tischhoehe': return 'Tischhöhe';
          case 'sitzhoehe': return 'Sitzhöhe';
          case 'tischplattengroesse': return 'Tischplattengröße';
          default: return attr;
        }
      });
              ...
              <div className="mt-8">
                ...
                {configurationError && (
                  <div className="mt-4 p-4 bg-blue-50 rounded-md border border-blue-200">
                    <p className="text-sm text-blue-800">{configurationError}</p>
                  </div>
                )}
